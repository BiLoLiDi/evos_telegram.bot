...
from keyboards.default.tel_nomer_kb import tel
...
@dp.callback_query_handler(text = "🗑 Savatni tozalash", state = UserState.mahsulot_tanlash)
async def mahsulot_callback_query(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
...
async def buyurtma_berish(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer('Bolimni tanlang', reply_markup=make_categories_kb())
    await UserState.product_selection.set()
    await call.message.delete()


@dp.callback_query_handler(text = '', state = UserState.buyurtma_berish)
async def buyurtma_berih(call: CallbackQuery, state: FSMContext):
    tg_id = call.from_user.id
    db.add_order()
